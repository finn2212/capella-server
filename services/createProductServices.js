...
const getProperties = (musicSheetProject) => {
    ...
    // Project Types
    if (musicSheetProject.projectType >= 1 && musicSheetProject.projectType <= 3) {
      ids.push({ id: propertyMappings.projectTypes[musicSheetProject.projectType].id });
    }
    ...
    if (propertyMappings.paperFormatMappings[musicSheetProject.paperFormat]) {
      ids.push({ id: propertyMappings.paperFormatMappings[musicSheetProject.paperFormat].id });
    }
    return ids;
  };
  ...
  const createProduct = async (accessToken, productData) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) { // Better way to check if error is an Axios error
        console.error("Product creation error:", error.response ? error.response.data : error.message);
      } else {
        console.error("An unknown error occurred during product creation:", error);
      }
  ...
  const fetchProduct = async (productNumber) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        console.error("Fetching product error:", error.response ? error.response.data : error.message);
      } else {
        console.error("An unknown error occurred during product fetching:", error);
      }
      throw error; // Rethrow the error to handle it outside this function if needed
    }
  };
